              ...
              <tbody className="divide-y divide-slate-100">
                {customer.contracts.map((c, i) => (
                  <tr key={c.id} className="hover:bg-slate-50 transition-colors animate-slide-in" style={{ animationDelay: `${i * 40}ms` }}>
                    ...
                    <td className="px-4 py-2.5 text-sm text-slate-600">{t(`billing.${c.billingFrequency}`) || c.billingFrequency}</td>
                    <td className="px-4 py-2.5 text-sm text-slate-600">{t(`billing.${c.paymentModel}`) || c.paymentModel}</td>
