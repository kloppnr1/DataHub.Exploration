...
A functional description of the DataHub settlement system — what it does, not how it's built.
...
### What the customer does NOT choose
...
### How pricing relates to the invoice
...
All DataHub messages use CIM (Common Information Model) JSON format. This is a standardized energy industry format defined by Energinet. Each message contains:
- A market document header (sender, receiver, timestamps)
- One or more series (each identified by GSRN)
- Per series: a period with resolution (PT1H = hourly) and data points

The system parses these into domain objects and stores the relevant data. Unknown fields are ignored — the parser is tolerant of format additions.
